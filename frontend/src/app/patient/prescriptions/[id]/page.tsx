...
import { getPrescriptionByIdPatient } from '@/app/shared/api/patient.api'
...
export default function Page() {
  ...
  useEffect(() => {
    const fetchPrescription = async () => {
      try {
        const res = await getPrescriptionByIdPatient(prescriptionId)
  ...
  return (
    <div className="flex justify-center mt-5 px-4 sm:px-6 md:px-8">
      <div className="bg-white max-w-xl w-full rounded-xl shadow-lg overflow-hidden min-h-[500px] text-sm">
        ...
        <div className="flex justify-between items-start bg-cyan-600 text-white px-4 py-3 rounded-t-xl">
          ...
          <div className="text-right">
            ...
            <p className="text-xs">{doctor?.qualification?.join(", ") || "MBBS"}</p>
            {doctor?.specialization && <p className="text-xs">{doctor.specialization}</p>}
          </div>
        </div>

        {/* Patient Info */}
        <div className="px-5 py-3 border-b border-gray-300 flex flex-wrap justify-between gap-2 text-black text-sm">
          <span><b>Name:</b> {patient?.name || "—"}</span>
          <span><b>Age:</b> {calculateAge(patient?.dob)}</span>
