...
import { getAdminNotificationsApi, markAllNotificationsReadAdmin, markNotificationReadAdmin } from '@/app/shared/api/admin.api'
...
export default function Page() {
  ...
  const handleMarkRead = async (n_id: string) => {
    try {
      await markNotificationReadAdmin(n_id)
  ...
  const handleReadAll = async () => {
    try {
      await markAllNotificationsReadAdmin()
      setNotifications(prev =>
  ...
  return (
    <div className="min-h-screen bg-gray-950 p-10 flex flex-col items-center space-y-10">
      ...
      {notifications.map((notif) => {
        return (
          <div
          onClick={()=> notif.leaveId && router.push("/admin/leaves") }
            key={notif._id}
